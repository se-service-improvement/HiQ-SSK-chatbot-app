while getopts ":s:o:a:" opt; do
  case $opt in
***REMOVED***s) search_service_resource_id="$OPTARG"
***REMOVED***;;
***REMOVED***o) azure_openai_resource_id="$OPTARG"
***REMOVED***;;
***REMOVED***a) storage_account_resource_id="$OPTARG"
***REMOVED***;;
***REMOVED***\?) echo "Invalid option -$OPTARG" >&2
***REMOVED***;;
  esac
done
...
***REMOVED***get_azure_openai_resource_system_assigned_identity_principal_id $resource_id
  elif [[ "$resource_type" == "Microsoft.Search" ]]; then
***REMOVED***get_azure_search_resource_system_assigned_identity_principal_id $resource_id
  else
***REMOVED***echo "Unknown resource type $resource_type" 1>&2
***REMOVED***exit 1
  fi
}
...
***REMOVED***--assignee-object-id $principal_id \
***REMOVED***--assignee-principal-type ServicePrincipal \
***REMOVED***--role "$role" \
***REMOVED***--scope "$resource_id" \
***REMOVED***--subscription $(get_subscription_id $resource_id)
}
...
function ensure_role_assignment_for_me() {
  ...
  echo "ensure role assignment $role for $assignee on $resource_id"
  az role assignment create \
***REMOVED***--assignee-object-id $assignee \
***REMOVED***--assignee-principal-type User \
***REMOVED***--role "$role" \
***REMOVED***--scope "$resource_id" \
***REMOVED***--subscription $(get_subscription_id $resource_id)
}
